...
import { connectorCodeLogin, type ConnectFormState } from "@/app/actions/connect";

const initial: ConnectFormState = {};

export function ConnectLoginForm() {
  const [state, formAction, pending] = useActionState(connectorCodeLogin, initial);
  ...
  return (
    <form action={formAction} className="mx-auto mt-8 max-w-sm space-y-4">
      <label className="flex flex-col gap-1.5 text-sm">
        <span className="font-medium text-stone-800">Connector access code</span>
        <input
          name="access_code"
          required
          autoComplete="off"
